...
; $(function () {
    ...
    $.GetAndDisplayLocations = function (country) {
		...
		//Figures out which radio button was selected, and sets the distance based off that.
		var distance = "";
		if($('#distance50').is(':checked')){
			distance = 50;
		}
		else if($('#distance100').is(':checked')){
			distance = 100;
		}
		else if($('#distance250').is(':checked')){
			distance = 250;
		}
		if(distance != ''){
			$('#distError').hide();
		...
		if (zip!= null){
			...
			$.get(googleapi, function (data) {
				if (data.status === "OK") {
					...
					$.crud($.service('/forcematch', 'ForceMatchByZip?' + filterUSB), 'get')
