...
class Things extends BasicAdmin {

    private $dataform = 'things_title';
...
//    private $cate = 'news_cate';
    ...
    public function index() {
        ...
        (isset($get['keywords']) && $get['keywords'] !== '') && $db->whereLike('small_title|title', "%{$get['keywords']}%");
...
//            $db->whereBetween('create_at', [$start_time, $end_time]);
    ...
    protected function _data_filter(&$data) {
        foreach ($data as $key => $val) {
            $data[$key]['chanping'] = Db::name('chanping')->where('id', '=', $val['cid'])->value('cp_title');
        }
    }
    /**
     * 添加
    ...
    protected function _form_result($result) {
        if ($result !== false) {
            list($base, $spm, $url) = [url('@admin'), $this->request->get('spm'), url('tuanj/things/index')];
